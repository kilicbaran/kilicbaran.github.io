<script context="module">
    ...
    export async function load({ page, fetch }) {
        ...
        posts.filter(post => post.metadata.tags.includes(slug));
        return {
...
<div class="p-2">
    ...
    >
        <h1
            class="text-7xl font-bold tracking-tighter leading-none"
        >
            Baran's Blog
        </h1>
        ...
        {#each posts.slice(0, 1) as { metadata, slug }}
            ...
            <div class="flex flex-col md:flex-row bg-gray-50 dark:bg-gray-800 border rounded-xl overflow-hidden items-center">
                ...
                <div class="w-full md:w-1/2 p-4 space-y-4">
                    ...
                    <div class="text-sm">{new Date(metadata.date).toLocaleDateString("en-US", {year: "numeric",month: "short",day: "numeric"})}</div>
                </div>
            </div>
            </a>
        {/each}
